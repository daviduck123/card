...
abstract class BaseSkill extends BaseObject  implements Persistent {
	...
	public function setNama($v)
	{
		...
		if ($this->nama !== $v) {
			$this->nama = $v;
			$this->modifiedColumns[] = SkillPeer::NAMA;
		}
	...
	public function setDeskripsi($v)
	{
		...
		if ($this->deskripsi !== $v) {
			$this->deskripsi = $v;
			$this->modifiedColumns[] = SkillPeer::DESKRIPSI;
		}
	...
	public function setCreatedAt($v)
	{
		...
		if ($this->created_at !== $ts) {
			$this->created_at = $ts;
			$this->modifiedColumns[] = SkillPeer::CREATED_AT;
		}
	...
	public function setUpdatedAt($v)
	{
		...
		if ($this->updated_at !== $ts) {
			$this->updated_at = $ts;
			$this->modifiedColumns[] = SkillPeer::UPDATED_AT;
		}
	...
	{
    if ($this->isNew() && !$this->isColumnModified(SkillPeer::CREATED_AT))
    {
      $this->setCreatedAt(time());
    }

    if ($this->isModified() && !$this->isColumnModified(SkillPeer::UPDATED_AT))
    {
	...
	protected function doSave($con)
	{
		$affectedRows = 0; 		if (!$this->alreadyInSave) {
			$this->alreadyInSave = true;
						...
						if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = SkillPeer::doInsert($this, $con);
					$affectedRows += 1; 										 										 
					$this->setId($pk);  
					$this->setNew(false);
				} else {
					$affectedRows += SkillPeer::doUpdate($this, $con);
				}
				$this->resetModified(); 			}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SkillPeer::NAMA)) $criteria->add(SkillPeer::NAMA, $this->nama);
		if ($this->isColumnModified(SkillPeer::DESKRIPSI)) $criteria->add(SkillPeer::DESKRIPSI, $this->deskripsi);
		if ($this->isColumnModified(SkillPeer::CREATED_AT)) $criteria->add(SkillPeer::CREATED_AT, $this->created_at);
		if ($this->isColumnModified(SkillPeer::UPDATED_AT)) $criteria->add(SkillPeer::UPDATED_AT, $this->updated_at);
